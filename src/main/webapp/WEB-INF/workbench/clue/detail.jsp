			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						...
						<tbody id="clueRelation">
						</tbody>
					</table>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-primary" onclick="bind()" data-dismiss="modal">关联</button>
				</div>
			</div>
		</div>
	</div>
...
<script>
    refreshClueRelationActivity();
    function refreshClueRelationActivity() {
        $.get("${pageContext.request.contextPath}/workbench/clue/clueActivityList",{
            'id':'${id}'
        },function (data) {
            var activityList = data;
        //    清空列表
            $('#clueRelationActivity').html("");
            for (var i = 0;i<activityList.length;i++) {
                var activity = activityList[i];
            //    拼出线索关联的市场活动列表
                $('#clueRelationActivity').append("<tr>\n" +
                    "\t\t\t\t\t\t\t\t<td>"+activity.name+"</td>\n" +
...
//    给搜索框增加回车事件以便触发触发搜索请求
    $('#searchInput').keypress(function (event) {
        if (event.keyCode ==13) {
        //    按名称模糊查询未被绑定的市场活动
            $.get("${pageContext.request.contextPath}/workbench/clue/queryClueActivityList",
                {
                    'name': $(this).val(),
                    'id'  : '${id}'
                },function (data) {
            //    清空列表
                    ...
                    //    把查询到的数据拼到响应的模态框中的列表中
                        var activity = data[i]
                        $('#clueRelation').append("<tr>\n" +
...
//    绑定市场活动
    ...
    //发送请求绑定市场活动
        $.post("${pageContext.request.contextPath}/workbench/clue/bind",
            {
                'ids': ids.join(),
                'id'  : '${id}'
            },function (data) {
...
//    解除市场活动与线索的关联
    function unbind(id,$this) {
        $.get("${pageContext.request.contextPath}/workbench/clue/unbind",
            {
                'activityId':id,
                'clueId':'${id}'
            },function (data) {
