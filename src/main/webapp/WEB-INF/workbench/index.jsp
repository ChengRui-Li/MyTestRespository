...
<script type="text/javascript">
	...
	$(function(){
		...
		window.open("${pageContext.request.contextPath}/toView/workbench/main/index","workareaFrame");
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 70%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" id="userUpdate"  class="btn btn-primary" data-dismiss="modal" onclick="updateUser()">更新</button>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="modal fade" id="uploadImg" role="dialog">
        <div class="modal-dialog" role="document" style="width: 24%;">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" id="uploadImgButton"  class="btn btn-primary" data-dismiss="modal" onclick="updateUserFileUpload()">更新</button>
                </div>
            </div>
        </div>
    </div>
	...
	<div class="modal fade" id="exitModal" role="dialog">
		<div class="modal-dialog" role="document" style="width: 30%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-primary" data-dismiss="modal" onclick="window.location.href='${pageContext.request.contextPath}/settings/user/logout';">确定</button>
				</div>
			</div>
		</div>
	</div>
			...
			<ul>
				<li class="dropdown user-dropdown">
					...
					<ul class="dropdown-menu">
						...
						<li><a href="javascript:void(0)" data-toggle="modal" data-target="#uploadImg"><span class="glyphicon glyphicon-edit"></span> 更换头像</a></li>
	...
	<div id="center" style="position: absolute;top: 50px; bottom: 30px; left: 0px; right: 0px;">
		...
		<div id="navigation" style="left: 0px; width: 18%; position: relative; height: 100%; overflow:auto;">
			...
			<ul id="no1" class="nav nav-pills nav-stacked">
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/main/index" target="workareaFrame"><span class="glyphicon glyphicon-home"></span> 工作台</a></li>
				...
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/activity/index" target="workareaFrame"><span class="glyphicon glyphicon-play-circle"></span> 市场活动</a></li>
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/clue/index" target="workareaFrame"><span class="glyphicon glyphicon-search"></span> 线索（潜在客户）</a></li>
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/customer/index" target="workareaFrame"><span class="glyphicon glyphicon-user"></span> 客户</a></li>
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/contacts/index" target="workareaFrame"><span class="glyphicon glyphicon-earphone"></span> 联系人</a></li>
				<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/transaction/index" target="workareaFrame"><span class="glyphicon glyphicon-usd"></span> 交易（商机）</a></li>
				...
				<li class="liClass">
					...
					<ul id="no2" class="nav nav-pills nav-stacked collapse">
						...
						<li class="liClass"><a href="${pageContext.request.contextPath}/toView/workbench/chart/transaction/index" target="workareaFrame">&nbsp;&nbsp;&nbsp;<span class="glyphicon glyphicon-chevron-right"></span> 交易统计图表</a></li>
					</ul>
				</li>
    ...
    <script>
        $('#oldPwd').blur(function () {
            $.post("${pageContext.request.contextPath}/sittings/user/verifyOldPwd",
                {"oldPwd" : $(this).val()},
                function (data) {
                    var resultVo = data;
                    if (!resultVo.ok) {
                        layer.alert(resultVo.message, {icon: 5});
                        $('#userUpdate').prop("disabled","disabled");
                    }
            },'json');
        });
        $('#newPwd').blur(function(){
            ...
            if (!($('#confirmPwd').val() == "" || $('#newPwd').val() == "")) {
                ...
                if ($('#newPwd').val() != $('#confirmPwd').val()) {
                    layer.alert("两次密码输入的不一致,请重新输入", {icon: 5});
                    $('#userUpdate').prop("disabled","disabled");
                } else {
                    $('#userUpdate').removeProp("disabled");
                }
            }
        });
        $('#confirmPwd').blur(function(){
            if (!($('#confirmPwd').val() == "" || $('#newPwd').val() == "")) {
                if ($('#newPwd').val() != $('#confirmPwd').val()) {
                    layer.alert("两次密码输入的不一致请重新输入", {icon: 5});
                    $('#userUpdate').prop("disabled","disabled")
                } else {
                    $('#userUpdate').removeProp("disabled")
                }
        ...
        function updateUser() {
            $.post("${pageContext.request.contextPath}/settings/user/updateUser",
                {
                'loginPwd' : $('#newPwd').val()
        ...
        $('#img').change(function () {
            // alert("方法已经执行")
            $.ajaxFileUpload({
                    url: '${pageContext.request.contextPath}/settings/user/fileUpload', //用于文件上传的服务器端请求地址
        ...
        function updateUserFileUpload() {
            alert("数据库写入头像文件名方法执行成功")
            $.post("${pageContext.request.contextPath}/settings/user/updateUserFileUpload",
                {
                    'img' : $('#photoPath').val()
